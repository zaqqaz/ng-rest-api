...
/***/ function(module, exports, __webpack_require__) {
	...
	angular.module('app', ['ng-rest-api']).config(["apiProvider", function (apiProvider) {
	    apiProvider.setBaseRoute('http://mock.local/api/');
	    apiProvider.setCacheDefaultLifetime(1000);

	    apiProvider.endpoint('course').route('courses/:id').addHttpAction('GET', 'query', {
	        isArray: true,
	        $cache: { lifetime: 2000 },
	        headersForReading: ['x-total-count'],
	        params: { limit: 10 }
	    }).addHttpAction('PATCH', 'patch', { params: { id: '@id' } }).addHttpAction('DELETE', 'remove', { params: { id: '@id' } });
	}]).controller('app', ["api", "$interval", function (api, $interval) {
	    var counter = 0;
	    $interval(function () {
	        api.course.query().then(function (r) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _api_endpoint_config = __webpack_require__(3);

	var _api_endpoint_config2 = _interopRequireDefault(_api_endpoint_config);

	var _api_endpoint = __webpack_require__(4);
